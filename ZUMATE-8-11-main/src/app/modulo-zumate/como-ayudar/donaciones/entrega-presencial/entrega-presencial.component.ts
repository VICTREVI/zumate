...
@Component({
  selector: 'app-entrega-presencial',
  templateUrl: './entrega-presencial.component.html',
  styleUrls: ['./entrega-presencial.component.css']
})
export class EntregaPresencialComponent {
  ...
  constructor(private fb: FormBuilder, private entregaService: EntregaPresencialService) {
  ...
  onSubmit() {
    if (this.form.valid) {
      ...
      this.entregaService.enviarFormulario(formData).pipe(
        catchError(error => {
